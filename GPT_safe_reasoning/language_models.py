    ...
    def generate(self, conv: List[Dict], 
        ...
        for _ in range(self.API_MAX_RETRY):
            try:
                ...
                print(f"""\n{'=' * 80}\n Output: {output} \n{'=' * 80}\n""")
                if not re.search('<analysis>.*?</analysis>', output) or not re.search('<response>.*?</response>', output):
                    out_dic = common._extract_json(output)
                    if out_dic == None:
                        output = f""" <analysis>Error</analysis>  <response>1</response> """
                        print(f"""\n{'=' * 80}\n Error Output: {output} \n{'=' * 80}\n""")
                    else:
                        analysis = out_dic["analysis"]
                        response = out_dic["response"]
                        output = f""" <analysis>{analysis}</reason>  <response>{response}</response> """
                        print(f"""\n{'=' * 80}\n Dict Output: {output} \n{'=' * 80}\n""")
                break
            except Exception as e:
            #except openai.error.OpenAIErrorException as e:
                #print(type(e), e)
