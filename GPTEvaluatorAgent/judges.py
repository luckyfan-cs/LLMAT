...
class EvidentJudge():
    def __init__(self, args):
        ...
        self.num_streams = args.n_streams
    ...
    def _get_scores_reasons(self,adv_prompt_list, target_response_list):
        ...
        judge_reasons = []
        for judge_expert in self.judge_experts:
            score_outputs,reason_outputs = judge_expert.score_reason(adv_prompt_list, target_response_list)
    ...
    def _get_evidences(self,adv_prompt_list, target_response_list):
        ...
        -------
        """
        judge_scores, judge_reasons = self._get_scores_reasons(adv_prompt_list, target_response_list)
    ...
    def get_decision_scores_reasons(self, adv_prompt_list, target_response_list):
        scores = []
        judge_evidences, judge_reasons = self._get_evidences(adv_prompt_list, target_response_list)
        evidences = judge_evidences[0]
        expert_reasons = ""
        for expert_id, reason in enumerate(judge_reasons):
            expert_reasons += "\n The expert {}'s score is {} and the reason is ".format(expert_id,evidences[expert_id][0] * self.base_num)+ " " + reason[0]
        print("\nnum of experts: \n", len(judge_reasons))
